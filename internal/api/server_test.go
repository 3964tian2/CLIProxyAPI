...
func newTestServerWithConfig(t *testing.T, cfg *proxyconfig.Config, authManager *auth.Manager, opts ...ServerOption) *Server {
	...
	t.Cleanup(func() {
		if errClose := server.closeUsageLedger(); errClose != nil {
			t.Errorf("close usage ledger: %v", errClose)
		}
	})
	return server
}
...
func TestModelsWithClientVersionReturnsCodexCatalog(t *testing.T) {
	...
	if got := int(codexClientTestPriority(custom["priority"])); got <= 0 {
		t.Fatalf("custom priority = %v, want a positive priority", custom["priority"])
	}
