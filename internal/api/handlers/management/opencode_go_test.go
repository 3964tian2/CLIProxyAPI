...
func TestOpenCodeGoRefreshUsageFetchesFromOpenCode(t *testing.T) {
	...
	h := &Handler{cfg: &config.Config{
		OpenCodeGo: config.OpenCodeGoConfig{
			Accounts: []config.OpenCodeGoAccount{{ID: "acc_1", Cookie: "session=secret", APIKey: "sk-open-1"}},
		},
	...
	for _, event := range []usageledger.Event{
		{
			RequestID:  "opencode-current",
			Timestamp:  now,
			Provider:   "openai-compatible-opencode-go",
			Model:      "claude-sonnet-4",
			APIKeyHash: usageledger.HashAPIKey("client-key-a"),
			AccountRef: openCodeGoProviderKeySource("acc_1"),
			Tokens:     usageledger.TokenUsage{TotalTokens: 15},
		},
		{
			RequestID:  "opencode-current-other-client",
			Timestamp:  now,
			Provider:   "openai-compatible-opencode-go",
			Model:      "claude-sonnet-4",
			APIKeyHash: usageledger.HashAPIKey("client-key-b"),
			AccountRef: openCodeGoProviderKeySource("acc_1"),
			Tokens:     usageledger.TokenUsage{TotalTokens: 6},
		},
		{
			RequestID:  "opencode-other",
			Timestamp:  now,
			Provider:   "openai-compatible-opencode-go",
			Model:      "claude-sonnet-4",
			APIKeyHash: usageledger.HashAPIKey("client-key-a"),
			AccountRef: openCodeGoProviderKeySource("acc_2"),
			Tokens:     usageledger.TokenUsage{TotalTokens: 90},
		},
	} {
		if _, err := store.InsertEvent(context.Background(), event); err != nil {
			t.Fatalf("insert cpa usage event: %v", err)
		}
	}
	...
	if body.Account.CPAUsage.Rolling.Tokens.TotalTokens != 21 ||
		body.Account.CPAUsage.Weekly.Tokens.TotalTokens != 21 ||
		body.Account.CPAUsage.Monthly.Tokens.TotalTokens != 21 {
		t.Fatalf("cpa usage = %#v", body.Account.CPAUsage)
	}
}
